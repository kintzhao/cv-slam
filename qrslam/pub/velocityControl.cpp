...
int  offset_xy = 0.002;
int  offset_z  = 1.0/360*3.141592653589;
...
void chatterCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    velocityControl(x,y,theta);
}
bool velocityControl(double x, double y, double theta)
{
    int length = 1.0;
    int width = 1.0;
    int turn = CV_PI/4.0;
    if( (abs(x - length) >= offset_xy) && (abs(y) <= offset_xy) && (abs(theta) < turn*0 + offset_z)  )  //  length
    {
        vel.angular.z = 0.0;
        vel.linear.x  = 0.1;
        vel.linear.y  = 0;
    ...
    if( (abs(x - length) < offset_xy) && (abs(y) <= offset_xy) && (abs(theta) < turn*1 + offset_z)  )  //  length
    {
        vel.angular.z = 0.1;
        vel.linear.x  = 0.0;
        vel.linear.y  = 0;
    ...
    if( (abs(x - length) < offset_xy) && (abs(y - width ) >= offset_xy) && (abs(theta - turn*1  ) > offset_z)  )  //  length
    {
        vel.angular.z = 0.1;
        vel.linear.x  = 0.0;
        vel.linear.y  = 0;
