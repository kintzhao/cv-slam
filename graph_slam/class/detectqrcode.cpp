...
ofstream fthreshold2("threshold.txt");
ofstream fPixPos2("pixpos.txt");
ofstream fCalc_info2("Calc_info.txt"); //采集mark的解析值
ofstream fConpensate_img_process2("./conpensate/fConpensate_img_process2.txt"); //采集mark的解析值
ofstream fThreshold_img_process2("./conpensate/fThreshold_img_process2.txt"); //采集mark的解析值

DetctQrcode::DetctQrcode(char * mapFile, int numForInit)
{
    useBCH = false;
    ...
    if (!readMarkForLocalization(numForInit))
    {
        cout<<"check the message of mark 0 in configure"<<endl;
       // waitKey();
    }
    undistort_ = atoi( input_data_[std::string("isundistort")].c_str() ) ; //是否加入图像去畸变

    calibFile = input_data_[std::string("calibFile")].c_str();
    ...
    //相机焦距相关
    camInnerPara.fx = camera_matrix_.at<double>(0,0)/100.0;
    camInnerPara.fy = camera_matrix_.at<double>(1,1)/100.0;

    if (undistort_)          //    #define undistort 1
    {
        camInnerPara.dx = camera_matrix_.at<double>(0,2);
        camInnerPara.dy = camera_matrix_.at<double>(1,2);
    }
    else
    {
        camInnerPara.dx = 320;
    ...
    middle_.x = (int)camInnerPara.dx;
    middle_.y = (int)camInnerPara.dy;
    ...
    fPixPos2 <<"id corn0_x y corn1_x y coen2_x y corn3_x y center_x y  …… "<<endl;
...
//    cv::line(show_landmark_img_,Point(0,240),Point(640,240),CV_RGB(0,0,0),1,8);
    ...
    cv::rectangle(show_landmark_img_,Point(middle_.x-1,middle_.y-1),Point(middle_.x+1,middle_.y+1),CV_RGB(0,255,0),1,8); //圈取图像中心点
    cv::line(show_landmark_img_,Point(middle_.x,0),Point(middle_.x,480),CV_RGB(0,0,0),1,8);
    cv::line(show_landmark_img_,Point(0,middle_.y),Point(640,middle_.y),CV_RGB(0,0,0),1,8);
...
//    if (!p_tracker_marks_->init("./data/no_distortion.cal", 1.0f, 1000.0f)) // load MATLAB file
    if (!p_tracker_marks_->init(calibFile.c_str(), 1.0f, 1000.0f)) // load MATLAB file
    {
    ...
    p_tracker_marks_->activateAutoThreshold(true);
...
{
    Point2i middle(image.rows,image.cols);
    found_code = false;
    loopClear();
    if (undistort_)          //    #define undistort 1
       cv::remap(image, image, map1_, map2_, cv::INTER_LINEAR);
    ...
    // trackerFunction(p_tracker_raw_marks_); // 下面qr提取前期一系列的操作的封装　　ARToolkitplus 处理二维码信息  1000次自动track
    p_tracker_raw_marks_->setPixelFormat(ARToolKitPlus::PIXEL_FORMAT_LUM);
    //    if (!p_tracker_raw_marks_->init("./data/no_distortion.cal", 1.0f, 1000.0f)) // load MATLAB file
    if (!p_tracker_raw_marks_->init(calibFile.c_str(), 1.0f, 1000.0f)) // load MATLAB file
    {
        printf("ERROR:p_tracker_raw_marks_->init（） failed\n");
    }
    //printSettings
    p_tracker_raw_marks_->setPatternWidth(2.0);
    p_tracker_raw_marks_->setBorderWidth(useBCH ? 0.125 : 0.25);    //  const bool useBCH = false;
    p_tracker_raw_marks_->setUndistortionMode(ARToolKitPlus::UNDIST_LUT);
    p_tracker_raw_marks_->setMarkerMode(useBCH ? ARToolKitPlus::MARKER_ID_BCH : ARToolKitPlus::MARKER_ID_SIMPLE);
    /// Turns automatic threshold calculation on/off
    p_tracker_raw_marks_->activateAutoThreshold(true);
    p_tracker_raw_marks_->setNumAutoThresholdRetries(1000);
    ...
    {
        tryCount = 300;
        nMarker_info = NULL;    //mark   记得清空
        nNumMarkers = 0;
    //     p_tracker_raw_marks_->setThreshold(tryCount);
        cout<<""<<gray.cols<<"  "<<gray.rows<<"    "<<endl;
//        cv::imshow(" gray ",gray);
//        cv::waitKey(20);
        std::vector<int> markerId = p_tracker_raw_marks_->calc(gray.data, &nMarker_info, &nNumMarkers);

        cout<<"nNumMarkers"<<nNumMarkers<<"  "<<markerId.size()<<endl;
        int value_thre =  p_tracker_raw_marks_->getThreshold();
        fthreshold2<<"  "<< value_thre; // 二值化阈值
        tryCount++;
        tryCount = tryCount + 5;
        for (int i = 0 ; i < nNumMarkers ; i++)
        {
            ...
            if ( (nMarker_info[i].id != -1) && (ht_[nMarker_info[i].id] > 0.1) )
            {
                ...
                fCalc_info2<<" area cf dir id line pos vertex "<<endl;
                cout<<"nMarker_info[i].id "<<nMarker_info[i].id<<endl;
                ...
                for (int ii = 0 ; ii < detectedID.size(); ii++)
                {
                    fCalc_info2<<" "<<Mark_info.area<<" "<<Mark_info.cf<<" "<<Mark_info.dir
                             <<" "<<Mark_info.id<<" "<<endl;

                    fCalc_info2<<" "<<Mark_info.line[(4-Mark_info.dir+0)%4][0]<<" "<<Mark_info.line[(4-Mark_info.dir+0)%4][1]<<" "<<Mark_info.line[(4-Mark_info.dir+0)%4][2]
                    ...
                    fCalc_info2<<" "<<Mark_info.pos[0]<<" "<<Mark_info.pos[1]<<endl;

                    fCalc_info2<<" "<<Mark_info.vertex[(4-Mark_info.dir+0)%4][0]<<" "<<Mark_info.vertex[(4-Mark_info.dir+0)%4][1]
                              ...
                              <<" "<<Mark_info.vertex[(4-Mark_info.dir+3)%4][0]<<" "<<Mark_info.vertex[(4-Mark_info.dir+3)%4][1]<<endl;
...
//  scale[0] = ((side_sizes_[id]/side[0]) + (side_sizes_[id]/side[3])) / 2.0f;
//  scale[1] = ((side_sizes_[id]/side[1]) + (side_sizes_[id]/side[0])) / 2.0f;
//  scale[2] = ((side_sizes_[id]/side[2]) + (side_sizes_[id]/side[1])) / 2.0f;
//  scale[3] = ((side_sizes_[id]/side[3]) + (side_sizes_[id]/side[2])) / 2.0f;
...
bool DetctQrcode::codeTestProcess(Mat image_raw )
{
    bool flag = false;
    //加载图片与灰度化化图像
    //cv::Mat image_raw = cvLoadImage("./input/cmpCut41.jpg", CV_LOAD_IMAGE_UNCHANGED);
    cv::Mat image_gray = image_raw.clone();  //= cvCreateImage(cvSize(image_raw->width,image_raw->height),IPL_DEPTH_8U,1);
    cvtColor(image_raw, image_gray,CV_BGR2GRAY);//灰度化


    imshow("testwindow",image_gray);
    int width = image_gray.cols;
    int height = image_gray.rows;

    // 灰度化图像二值化
//    for(int threshold = 40;threshold<=150;threshold++)
//    {
//        threshold+=150;
        //image_gray2 = cvCreateImage(cvSize(image_gray->width,image_gray->height),IPL_DEPTH_8U,1);

        cv::Mat image_gray_threshold = image_gray.clone();
       // cv::threshold(image_gray,image_gray_threshold,threshold,255,CV_THRESH_BINARY); // 灰度二值化  阀值100
...
//        ftime(&time1);
        ...
        //ftime(&time2);
        //sec = (time2.time-time1.time)*1000.0 + (time2.millitm-time1.millitm);
        gettimeofday(&tim2, NULL);
        sec = (tim2.tv_sec - tim1.tv_sec)*1000000.0 + (tim2.tv_usec - tim1.tv_usec);//算法耗时说明 微秒
...
//        string text ="222";
//        cv::putText(image_gray_gradient,text,Point(40,40),CV_FONT_HERSHEY_COMPLEX,1,CV_RGB(0,0,255));
...
//        cv::imwrite(conpensate_img_name,image_gray_gradient);
        ...
        // load a camera file.
        if (! ptrackerWhole->init(calibFile.c_str(), 1.0f, 1000.0f)) // load MATLAB file
        {
            printf("ERROR: init() failed\n");
        }
         ptrackerWhole->setPatternWidth(2.0);
         ptrackerWhole->setBorderWidth(useBCH ? 0.125 : 0.25);    //  const bool useBCH = false;
         ptrackerWhole->setUndistortionMode(ARToolKitPlus::UNDIST_LUT);
         ptrackerWhole->setMarkerMode(useBCH ? ARToolKitPlus::MARKER_ID_BCH : ARToolKitPlus::MARKER_ID_SIMPLE);
         ...
         ftime(&time21);
         sec = (time21.time-time11.time)*1000.0 + (time21.millitm-time11.millitm);
         cout<<"  "<<sec<<"  "<<endl;
         ...
         for(int i = 0 ; i < ar_threshold_num ; i++)
         {
             if(ar_threshold_info[i].id != -1)
             {
                  ...
                  fThreshold_img_process2<<" "<<threshold<<" "<<Mark_info.id<<" "<<Mark_info.cf<<" "<<width<<" "<<height<<" "<<Mark_info.area<<" "<<Mark_info.dir<<" "
         ...
         for(int i = 0 ; i < ar_compensate_num ; i++)
         {
             if(ar_compensate_info[i].id != -1)
             {
                  ...
                  fConpensate_img_process2<<" "<<threshold<<" "<<Mark_info.id<<" "<<Mark_info.cf<<" "<<width<<" "<<height<<" "<<Mark_info.area<<" "<<Mark_info.dir<<" "
                                         <<" "<<Mark_info.pos[0]<<" "<<Mark_info.pos[1]
                           <<" "<<Mark_info.vertex[(4-Mark_info.dir+0)%4][0]<<" "<<Mark_info.vertex[(4-Mark_info.dir+0)%4][1]
                           <<" "<<Mark_info.vertex[(4-Mark_info.dir+1)%4][0]<<" "<<Mark_info.vertex[(4-Mark_info.dir+1)%4][1]
                           <<" "<<Mark_info.vertex[(4-Mark_info.dir+2)%4][0]<<" "<<Mark_info.vertex[(4-Mark_info.dir+2)%4][1]
...
{
    double centUoff = pos_world.x * camInnerPara.fy / ht_[id];  //camInnerFocus;
    double centVoff = pos_world.y * camInnerPara.fx / ht_[id] ; //camInnerFocus;

    double u_pix = camInnerPara.dy + centUoff;
    double v_pix = camInnerPara.dx - centVoff ;          //采取内参校正数值
...
//返回原始像素点坐标
vector<CPointsFour> DetctQrcode::detectRawLandmarksNew(cv::Mat image, int &MarkNum)
{
    Point2i middle(image.rows,image.cols);
    found_code = false;
    loopClear();
    ...
    ARMarkerInfo* pMarker_info;
    int mark_num;
    std::vector<int> markerId = p_tracker_marks_->calc(gray.data, &pMarker_info, &mark_num);
    cout<<"nNumMarkers"<<mark_num<<"  "<<markerId.size()<<endl;
    for (int i = 0 ; i < mark_num ; i++)
    {
        cout<<"pMarker_info[i].id "<<pMarker_info[i].id<<endl;
        if ( (pMarker_info[i].id != -1) && (ht_[pMarker_info[i].id] > 0.1) )
        {
            found_code = true;
            cout<<" found_code = true "<< pMarker_info[i].id<<endl;

            ARMarkerInfo  Mark_info = pMarker_info[i];
            ConerPoint  center,corner0,corner1,corner2,corner3 ;
            center.init(Mark_info.pos[0],Mark_info.pos[1]) ;
            corner0.init(Mark_info.vertex[(4-Mark_info.dir+0)%4][0],Mark_info.vertex[(4-Mark_info.dir+0)%4][1]) ;
            corner1.init(Mark_info.vertex[(4-Mark_info.dir+1)%4][0],Mark_info.vertex[(4-Mark_info.dir+1)%4][1]) ;
            ...
            corner3.init(Mark_info.vertex[(4-Mark_info.dir+3)%4][0],Mark_info.vertex[(4-Mark_info.dir+3)%4][1]) ;
            CPointsFour  c_temp;
            c_temp.init(corner0,corner1,corner2,corner3,center);
            coners_aver.push_back(c_temp);
        }
    }
    MarkNum = coners_aver.size() ;
    drawCoordinate(show_landmark_img_);
    drawQrcodeNew(pMarker_info) ;    //draw the square-qrcode_fourSide
    qr_landmark_cvt_->convertOnce(show_landmark_img_);  //display
    return coners_aver;
}


void DetctQrcode::drawQrcodeNew(ARMarkerInfo* pMarker_info)   //draw the square-qrcode_fourSide
{
    for (int count = 0 ; count < nNumMarkers; count++)
    {
        ARMarkerInfo Mark_new = pMarker_info[count];
        if (Mark_new.id != -1 && ht_[Mark_new.id] > 0.4 )
        {
            cv::line(show_landmark_img_,cv::Point(Mark_new.vertex[0][0],Mark_new.vertex[0][1]),cv::Point(Mark_new.vertex[1][0],Mark_new.vertex[1][1]),CV_RGB(255,0,0),1,8);
            cv::line(show_landmark_img_,cv::Point(Mark_new.vertex[1][0],Mark_new.vertex[1][1]),cv::Point(Mark_new.vertex[2][0],Mark_new.vertex[2][1]),CV_RGB(255,0,0),1,8);
            cv::line(show_landmark_img_,cv::Point(Mark_new.vertex[2][0],Mark_new.vertex[2][1]),cv::Point(Mark_new.vertex[3][0],Mark_new.vertex[3][1]),CV_RGB(255,0,0),1,8);
            cv::line(show_landmark_img_,cv::Point(Mark_new.vertex[3][0],Mark_new.vertex[3][1]),cv::Point(Mark_new.vertex[0][0],Mark_new.vertex[0][1]),CV_RGB(255,0,0),1,8);

            string dir_str = int2str(Mark_new.dir);
            string tx0 = "0";//int2str((4-Mark_new.dir+0)%4);
            string tx1 = "1";//int2str((4-Mark_new.dir+1)%4);
            string tx2 = "2";//int2str((4-Mark_new.dir+2)%4);
            string tx3 = "3";//int2str((4-Mark_new.dir+3)%4);
            if( 20 == Mark_new.id)
            {
               Mark_new.dir = Mark_new.dir +1;
               dir_str = int2str(Mark_new.dir);
               tx0 = "0";//int2str((4-Mark_new.dir+0)%4);
               tx1 = "1";//int2str((4-Mark_new.dir+1)%4);
               tx2 = "2";//int2str((4-Mark_new.dir+2)%4);
               tx3 = "3";//int2str((4-Mark_new.dir+3)%4);
            }
            cv::putText(show_landmark_img_,dir_str,cv::Point(400,400),CV_FONT_HERSHEY_COMPLEX,2,CV_RGB(0,0,255));
            cv::putText(show_landmark_img_,tx0,cv::Point(Mark_new.vertex[(4-Mark_new.dir+0)%4][0],Mark_new.vertex[(4-Mark_new.dir+0)%4][1]),CV_FONT_HERSHEY_COMPLEX,1,CV_RGB(255,0,0));
            cv::putText(show_landmark_img_,tx1,cv::Point(Mark_new.vertex[(4-Mark_new.dir+1)%4][0],Mark_new.vertex[(4-Mark_new.dir+1)%4][1]),CV_FONT_HERSHEY_COMPLEX,1,CV_RGB(255,0,0));
            cv::putText(show_landmark_img_,tx2,cv::Point(Mark_new.vertex[(4-Mark_new.dir+2)%4][0],Mark_new.vertex[(4-Mark_new.dir+2)%4][1]),CV_FONT_HERSHEY_COMPLEX,1,CV_RGB(255,0,0));
            cv::putText(show_landmark_img_,tx3,cv::Point(Mark_new.vertex[(4-Mark_new.dir+3)%4][0],Mark_new.vertex[(4-Mark_new.dir+3)%4][1]),CV_FONT_HERSHEY_COMPLEX,1,CV_RGB(255,0,0));

            std::string text ="ID_"+ int2str(Mark_new.id);
            cv::putText(show_landmark_img_,text,Point(Mark_new.pos[0]+80,Mark_new.pos[1]),CV_FONT_HERSHEY_COMPLEX,1,CV_RGB(255,0,0));
            cv::rectangle(show_landmark_img_,Point(Mark_new.pos[0]-1,Mark_new.pos[1]-1),Point(Mark_new.pos[0]+1,Mark_new.pos[1]+1),CV_RGB(0,255,0),1,8); //圈取图像中心点
        }
    }
